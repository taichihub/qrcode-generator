...
def test_logo_path():
  ...
  logo_files = [f for f in os.listdir(setting.IMG_DIRECTORY) if os.path.isfile(os.path.join(setting.IMG_DIRECTORY, f))]

  # 予想されるロゴファイルパスを設定（画像ファイルが存在する場合は最初のものを選択）
  expected_logo_path = os.path.join(setting.IMG_DIRECTORY, logo_files[0]) if logo_files else None
