...
def test_get_logo_image_path():
    ...
    logo_path = get_logo_image_path(PROGRAM_SETTINGS["DIRECTORY"]["IMG"])
    assert os.path.exists(logo_path) and logo_path.endswith(".png")


def test_create_subfolder():
    # サブフォルダを作成する機能をテスト
    subfolder_path = os.path.join(PROGRAM_SETTINGS["DIRECTORY"]["OUTPUT"], "test_subfolder")
...
def test_process_csv_file():
    csv_files = [
        f
        for f in os.listdir(PROGRAM_SETTINGS["DIRECTORY"]["INPUT"])
        if f.lower().endswith(".csv") and os.path.isfile(os.path.join(PROGRAM_SETTINGS["DIRECTORY"]["INPUT"], f))
    ]
    if not csv_files:
        test_process_csv_file_empty()
    for csv_file in csv_files:
        test_csv_path = os.path.join(PROGRAM_SETTINGS["DIRECTORY"]["data"], csv_file)
        run_process_csv_file(test_csv_path, PROGRAM_SETTINGS["DIRECTORY"]["OUTPUT"], PROGRAM_SETTINGS["DIRECTORY"]["IMG"])
        ...
        for index, row in df.iterrows():
            ...
            if use_numbering:
                qr_code_filename = os.path.join(
                    PROGRAM_SETTINGS["DIRECTORY"]["OUTPUT"], subfolder_name, str(
...
def test_process_csv_file_empty():
    # CSVファイルが空の場合の挙動をテスト
    empty_csv_path = os.path.join(PROGRAM_SETTINGS["DIRECTORY"]["INPUT"], "empty.csv")
    # 空のCSVファイルを作成
    with open(empty_csv_path, "w") as f:
        pass
    run_process_csv_file(empty_csv_path, PROGRAM_SETTINGS["DIRECTORY"]["OUTPUT"], PROGRAM_SETTINGS["DIRECTORY"]["IMG"])
...
def test_invalid_csv_data():
    invalid_csv_path = os.path.join(PROGRAM_SETTINGS["DIRECTORY"]["INPUT"], "invalid.csv")
    # 不正なデータを含むCSVファイルを作成
...
def test_no_png_in_img_directory():
    ...
    if not skip_if_no_logo:
        assert get_logo_image_path(PROGRAM_SETTINGS["DIRECTORY"]["IMG"]) is None
